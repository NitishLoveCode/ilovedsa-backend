...
import servicesBaseCompanyDSAStepsModal from "../../models/dashboard/servicesBaseCompanyDSASteps.modal";
import { IAuthTokenVerifyType, IDashboardResponseDataSet, IDsaStepsInterface } from "../../types/user.types";

export const servicesBaseCompanyDSAStepsServices = async (cookieData: IAuthTokenVerifyType): Promise<IDashboardResponseDataSet | null> => {
  try {

    // Getting dsa steps
    const dsaSteps = await servicesBaseCompanyDSAStepsModal.findAll();
