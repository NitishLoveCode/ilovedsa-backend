...
import { typescriptInterviewQuestion } from "../../models/dashboard/typescriptInterview.dashboard.models";
import { IAuthTokenVerifyType, IDashboardResponseDataSet, IDsaStepsInterface } from "../../types/user.types";

export const typescriptInterview = async (cookieData: IAuthTokenVerifyType): Promise<IDashboardResponseDataSet | null> => {
  try {

    // Getting dsa steps
    const dsaSteps = await typescriptInterviewQuestion.findAll();
