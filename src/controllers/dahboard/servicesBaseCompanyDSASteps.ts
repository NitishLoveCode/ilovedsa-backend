...
export const servicesBaseCompanyDSASteps = async (req: Request, res: Response, next: NextFunction) => {
    try{
        const dsaSteps = await servicesBaseCompanyDSAStepsServices(await verifyToken(req.cookies.token));
        if(dsaSteps){
            res
            .status(200)
            .json({
